...
# Custom Gated LNN Layer
class LNNStep(tf.keras.layers.Layer):
    def __init__(self, reservoir_weights, input_weights, gate_weights, leak_rate, **kwargs):
    ...
    def call(self, inputs, states):
        ...
        # Gate activations
        i_gate = tf.sigmoid(gate_part[:, :self.reservoir_dim])  # Input gate
        f_gate = tf.sigmoid(gate_part[:, self.reservoir_dim:2*self.reservoir_dim])  # Forget gate
        o_gate = tf.sigmoid(gate_part[:, 2*self.reservoir_dim:])  # Output gate

        # State calculation with gating
        state = (1 - self.leak_rate) * (f_gate * prev_state) + self.leak_rate * tf.tanh(i_gate * (input_part + reservoir_part))
        state = o_gate * state  # Apply output gate to the state
...
def create_gated_lnn_lstm_model(input_shape, reservoir_weights, input_weights, gate_weights, leak_rate, lstm_units, output_dim):
    inputs = Input(shape=input_shape)
    lnn_layer = tf.keras.layers.RNN(LNNStep(reservoir_weights, input_weights, gate_weights, leak_rate), return_sequences=True)
...
# Create Gated LNN-LSTM model
input_shape = (28, 28)
model = create_gated_lnn_lstm_model(input_shape, reservoir_weights, input_weights, gate_weights, leak_rate, lstm_units, output_dim)
...
# Gated Liquid Neural Network (GLNN)
# python glnn_mnist.py
