...
class GatedSpikingElasticLNNStep(layers.Layer):
    def __init__(self, reservoir_weights, input_weights, leak_rate, gate_weights, **kwargs):
    ...
    def call(self, inputs, states):
        ...
        gate_input = tf.matmul(inputs, self.gate_weights, transpose_b=True)
        gate = tf.sigmoid(gate_input)  # Gating mechanism
        
        input_part = tf.matmul(inputs, self.input_weights, transpose_b=True)
        reservoir_part = tf.matmul(prev_state, self.reservoir_weights)
        
        # Dynamic self-modeling
        state = (1 - self.leak_rate) * prev_state + self.leak_rate * gate * tf.tanh(input_part + reservoir_part)
        return state, [state]
...
def initialize_lnn_reservoir(input_dim, reservoir_dim, spectral_radius):
    """Initializes the reservoir and input weights for the D-SM-GSELNN."""
    reservoir_weights = np.random.randn(reservoir_dim, reservoir_dim)
    reservoir_weights *= spectral_radius / np.max(np.abs(np.linalg.eigvals(reservoir_weights)))
    input_weights = np.random.randn(reservoir_dim, input_dim) * 0.1
    gate_weights = np.random.randn(reservoir_dim, input_dim) * 0.1  # Gate weights
...
def build_hybrid_model(input_shape, reservoir_dim, spectral_radius, leak_rate, lstm_units):
    """Builds and compiles the Hybrid D-SM-GSELNN-LSTM model."""
    ...
    lnn_layer = GatedSpikingElasticLNNStep(
...
def main():
    ...
    # Step 4: Build and Train Hybrid D-SM-GSELNN-LSTM Model
    ...
    y_pred_inverse = scaler.inverse_transform(np.hstack((np.zeros((len(y_pred), scaled_data.shape[1] - 1)), y_pred)))[:, -1]

    # Step 7: Plot Results
    plot_predictions(scaler.inverse_transform(np.hstack((np.zeros((len(y_test), scaled_data.shape[1] - 1)), y_test)))[:, -1], y_pred_inverse)
